...
public class RiskGame {
	...
	public Vector<RiskContinent> continents = new Vector<RiskContinent>();
        ...
        public RiskPlayer curPlayer;
        public RiskPlayer defender;
      ...
      static public boolean addPlayer(String nm){
            ...
            if (size > 6)
                return false;
            RiskPlayer p = new RiskPlayer(nm, size);
            players.add(p);
                return true;
        }
      ...
      public void removePlayer(RiskPlayer p){
          players.remove(p);
          players.trimToSize();
          iter--;
	...
	return continentBonus;
      ...
      public void loadMap() {
  		...
  		try {
  			...
  			InputStream file = RiskGame.class
  			...
  			while (mapfile.hasNextLine()) {
  				...
  				if (next.equals("[Continents]")) {
  					...
  					do {
  						...
  						continents.add(new RiskContinent(name, contains, value));
  						next = mapfile.nextLine();
  				...
  				if (next.equals("[Territories]")) {
  					...
  					do {
  						if (!next.equals("-")) {
  							...
  							territories.add(new RiskTerritory(id, name, continent,
  									x, y));
  			...
  			while (mapfile.hasNextLine()) {
  				...
  				if (next.equals("[Territories]")) {
  					...
  					String[] all = mapfile.nextLine().split(",");
  					String c = all[0];
  					do {
      ...
      public Vector<RiskPlayer> getPlayers(){
          return players;
      }
